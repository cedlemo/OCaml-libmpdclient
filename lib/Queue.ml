...
let add client uri = Client.send_command client uri
      ...
      | Some song_id ->
          ...
          let rec parse lines =
            ...
            | line :: remain ->
                ...
                if k = "Id" then int_of_string v else parse remain
          in
          parse lines)
  | Protocol.Error _ -> -1

let clear client = Client.send_command client "clear"
...
let deleteid client id =
  Client.send_command client
    (String.concat " " [ "deleteid"; string_of_int id ])
...
let rec _build_songs_list client songs l =
  ...
  | h :: q -> (
      ...
      | Protocol.Error (_ack_val, _ack_cmd_num, _ack_cmd, ack_message) ->
          let message =
            Printf.sprintf "Song %s : %s " (get_song_pos h) ack_message
          in
...
let playlist client = _playlist_ client "playlist"
