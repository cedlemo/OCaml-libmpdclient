...
let unix_error_message (error, fn_name, param_name) user_str =
  let strs =
    [ Unix.error_message error; fn_name; param_name; user_str; ".Exiting..." ]
  in
...
let initialize hostname port =
  ...
  in
  let s =
    try Unix.socket PF_INET SOCK_STREAM 0
  ...
  in
  let _ =
    try Unix.connect s (Unix.ADDR_INET (ip, port))
    with Unix_error (error, fn_name, param_name) ->
      let custom_message =
        Printf.sprintf ": unable to connect to %s:%d" hostname port
      in
      unix_error_message (error, fn_name, param_name) custom_message
  in
...
let recvbytes t =
  ...
  with Unix.Unix_error (error, fn_name, param_name) -> (
    match error with
    | _ ->
