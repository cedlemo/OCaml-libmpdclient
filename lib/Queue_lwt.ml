...
type t = PlaylistError of string | Playlist of Song.t list

let add client uri = Client_lwt.send client uri
...
let clear client = Client_lwt.send client "clear"

let delete client position ?position_end () =
  let cmd =
    match position_end with
    | None -> String.concat " " [ "delete"; string_of_int position ]
    | Some pos_end ->
        String.concat ""
          [ "delete "; string_of_int position; ":"; string_of_int pos_end ]
  in
...
let move client position_from ?position_end position_to () =
  let cmd =
    ...
    | None ->
        String.concat " "
          [ "move"; string_of_int position_from; string_of_int position_to ]
    | Some pos_end ->
        String.concat ""
          [
            "move ";
...
let moveid client id position_to =
  Client_lwt.send client
    (String.concat " "
       [ "moveid"; string_of_int id; string_of_int position_to ])

let get_song_pos song =
  let pattern = "\\([0-9]+\\):file:.*" in
  let found = Str.string_match (Str.regexp pattern) song 0 in
  if found then Str.matched_group 1 song else "none"

let rec _build_songs_list client songs l =
  match songs with
  | [] ->
      let playlist = Playlist (List.rev l) in
      Lwt.return playlist
  | h :: q -> (
      let song_infos_request = "playlistinfo " ^ get_song_pos h in
      Client_lwt.request client song_infos_request >>= function
...
let playlistfind client tag needle =
  ...
  Client_lwt.request client request >>= fun response ->
  playlist_command_responses_handler client response
...
let swap client pos1 pos2 =
  Client_lwt.send client
    (String.concat " " [ "swap"; string_of_int pos1; string_of_int pos2 ])
...
let prio client priority ?range () =
  let priority' =
    string_of_int
      (if priority > 255 then 255 else if priority < 0 then 0 else priority)
  in
  let request =
    match range with
    | None -> "prio " ^ priority'
...
let swapid client id1 id2 =
  let request =
    String.concat " " [ "swapid"; string_of_int id1; string_of_int id2 ]
  in
  Client_lwt.send client request
...
let cleartagid client id tag =
  let request = String.concat " " [ "cleartagid"; string_of_int id; tag ] in
  Client_lwt.send client request
