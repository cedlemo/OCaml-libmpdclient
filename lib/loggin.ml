  ...
  let report src level ~over k msgf =
    let k () =
      let write () =
        ...
        >>= fun fd_err ->
        Lwt.return (fd_log, fd_err) >>= fun (fd_log', fd_err') ->
        match level with
        | Logs.App -> Lwt_io.write fd_log' (app_flush ())
        | _ ->
            Lwt_io.write fd_err' (dst_flush ()) >>= fun () ->
            Lwt_io.close fd_log' >>= fun () -> Lwt_io.close fd_err'
      in
      let unblock () =
        over ();
        Lwt.return_unit
...
let debuf message = Logs_lwt.debug (fun m -> m "%s" message)
let err message = Logs_lwt.err (fun m -> m "%s" message)
