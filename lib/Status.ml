...
type t = {
  ...
  audio : string;
      (** sampleRate:bits:channels TODO : Maybe create a specific type later *)
...
let parse lines =
  let rec _parse pairs s =
    ...
    | p :: remain -> (
        ...
        | "consume" ->
            _parse remain { s with consume = Utils.bool_of_int_str v }
        | "playlist" -> _parse remain { s with playlist = int_of_string v }
        | "playlistlength" ->
            _parse remain { s with playlistlength = int_of_string v }
        ...
        | "time" ->
            _parse remain { s with time = v }
            (* TODO: !! mpd time format min:sec*)
        ...
        | "mixrampdelay" ->
            _parse remain { s with mixrampdelay = float_of_string v }
        | "audio" -> _parse remain { s with audio = v }
        | "updating_db" ->
            _parse remain { s with updating_db = int_of_string v }
        | "error" -> _parse remain { s with error = v }
        | _ -> _parse remain s)
  in
  _parse lines empty

let volume { volume = v; _ } = v
let repeat { repeat = r; _ } = r
let random { random = r; _ } = r
let single { single = s; _ } = s
let consume { consume = c; _ } = c
let playlist { playlist = p; _ } = p
let playlistlength { playlistlength = p; _ } = p
let state { state = s; _ } = s
let song { song = s; _ } = s
let songid { songid = s; _ } = s
let nextsong { nextsong = n; _ } = n
let nextsongid { nextsongid = n; _ } = n
let time { time = t; _ } = t
let elapsed { elapsed = e; _ } = e
let duration { duration = d; _ } = d
let bitrate { bitrate = b; _ } = b
let xfade { xfade = x; _ } = x
let mixrampdb { mixrampdb = m; _ } = m
let mixrampdelay { mixrampdelay = m; _ } = m
let audio { audio = a; _ } = a
let updating_db { updating_db = u; _ } = u
let error { error = e; _ } = e
