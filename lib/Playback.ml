...
let next client = Client.send_command client "next"
let previous client = Client.send_command client "previous"
let stop client = Client.send_command client "stop"
...
let play client songpos =
  Client.send_command client
    (String.concat " " [ "play"; string_of_int songpos ])

let playid client songid =
  Client.send_command client
    (String.concat " " [ "playid"; string_of_int songid ])
...
let seekcur client time =
  Client.send_command client
    (String.concat " " [ "seekcur"; string_of_float time ])
