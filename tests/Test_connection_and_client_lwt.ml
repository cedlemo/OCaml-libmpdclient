...
let init_client () =
  Cnx_lwt.initialize host port >>= fun connection ->
...
let test_connection_initialize _test_ctxt =
  ignore
    (Lwt_main.run
       ( Cnx_lwt.initialize host port >>= fun connection ->
         Cnx_lwt.hostname connection >>= fun h ->
         let () = assert_equal ~printer host h in
         Cnx_lwt.port connection >>= fun p ->
...
let test_client_send _test_ctxt =
  TU.run_test_lwt (fun client ->
      Clt_lwt.send client "ping" >>= fun response ->
      let () =
        match response with
        | Error _ ->
...
let test_client_send_bad_command _test_ctxt =
  TU.run_test_lwt (fun client ->
      Clt_lwt.send client "badcommand" >>= fun response ->
      let () =
        match response with
...
let test_client_banner _test_ctxt =
  TU.run_test_lwt (fun client ->
      ...
      Clt_lwt.mpd_banner client >>= fun banner ->
...
let test_client_status _test_ctxt =
  TU.run_test_lwt (fun client ->
      Clt_lwt.status client >>= function
      | Error _message ->
          let printer _ = "This should not have been reached" in
