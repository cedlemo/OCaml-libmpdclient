...
let test_music_database_find _test_ctxt =
  ...
  TU.run_test_on_playlist_lwt (fun client ->
      find client [ (Mpd_tag Artist, TU.artist) ] () >>= fun response ->
      let () =
        match response with
...
let test_music_database_findadd _test_ctxt =
  let open Mpd.Music_database_lwt in
  TU.run_test_on_playlist_lwt (fun client ->
      findadd client [ (Mpd_tag Artist, TU.artist) ] >>= fun response ->
      TU.queue_length_lwt client >>= fun len ->
      let () =
        match response with
        | Error (_, _, _, error) -> bad_branch error
        | Ok _ -> assert_equal ~printer:string_of_int 11 len
      in
      Lwt.return_unit)
...
let test_music_database_searchaddpl _test_ctxt =
  ...
  TU.run_test_on_playlist_lwt (fun client ->
      ...
      let get_playlist_number () =
        Mpd.Stored_playlists_lwt.listplaylists client >>= function
        | Error message ->
            let () = bad_branch message in
            Lwt.return (-1)
        | Ok playlists ->
            let len = List.length playlists in
            Lwt.return len
      in
      searchaddpl client new_playlist [ (Mpd_tag Artist, TU.bad_name_artist) ]
      >>= function
      | Error (_, _, _, error) ->
          let () = bad_branch error in
          Lwt.return_unit
      | Ok _ ->
          get_playlist_number () >>= fun len ->
          let () = assert_equal 3 len in
          Mpd.Stored_playlists_lwt.rm client new_playlist >>= fun _ ->
          get_playlist_number () >>= fun len' ->
...
let test_music_database_count _test_ctxt =
  let open Mpd.Music_database_lwt in
  TU.run_test_lwt (fun client ->
      count client [] ?group:(Some Artist) () >>= fun response ->
      let () =
        match response with
        | Error message -> bad_branch message
        | Ok counts ->
            assert_equal ~printer:string_of_int 1 (List.length counts)
      in
      Lwt.return_unit)
...
let test_music_database_list_title _test_ctxt =
  let open Mpd in
  TU.run_test_lwt (fun client ->
      Music_database_lwt.list client Title
        [ (Artist, TU.artist); (Album, TU.album) ]
      >>= fun response ->
      let () =
        match response with
        | Error message -> bad_branch message
