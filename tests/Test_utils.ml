...
let init_client () =
  ...
  let () =
    match Mpd.Music_database.update client None with
    | Error (_, _, _, message) ->
...
let ensure_playback_is_stopped client = ignore (Mpd.Playback.stop client)
let ensure_playlist_is_cleared client = ignore (Mpd.Queue.clear client)
...
let run_test_lwt f =
  ignore
    (Lwt_main.run
       ( init_client_lwt () >>= fun client ->
         f client >>= fun _ -> Mpd.Client_lwt.close client ))
...
let run_test_on_playlist_lwt f =
  run_test_lwt (fun client -> f client >>= fun _ -> Mpd.Queue_lwt.clear client)

let assert_state client s test_name =
  match Mpd.Client.status client with
  | Error message ->
      assert_equal
        ~printer:(fun s -> test_name ^ s)
        "Unable to get status" message
  | Ok status ->
      assert_equal
        ~printer:(fun s -> test_name ^ Mpd.Status.string_of_state s)
...
let check_state client s =
  ...
  | Ok status -> s == Mpd.Status.state status
