...
let song_pos =
  let doc =
    "Integer value that represents the position of a song in the current \
     playlist."
  in
...
let play_t =
  let doc = "Play the song at SONG_POS in the playlist" in
...
let play_id_t =
  let doc = "Play the song SONG_ID." in
  let man = [ `S Manpage.s_description; `P doc; `Blocks help_section ] in
...
let time =
  let doc =
    "Float value that could represents the length of a song or the starting \
     point to play"
  in
...
let seek_t =
  let doc = "Play the song at SONG_POS in the playlist at TIME" in
  let man = [ `S Manpage.s_description; `P doc; `Blocks help_section ] in
  ( Term.(const seek $ common_opts_t $ song_pos $ time),
    Cmd.info "seek" ~doc ~sdocs ~exits ~man )

let seek_id_t =
  let doc = "Play the song SONG_ID at TIME" in
  let man = [ `S Manpage.s_description; `P doc; `Blocks help_section ] in
...
let next_t =
  let doc = "Play the next song in the playlist" in
  let man = [ `S Manpage.s_description; `P doc; `Blocks help_section ] in
  (Term.(const next $ common_opts_t), Cmd.info "next" ~doc ~sdocs ~exits ~man)
...
let previous_t =
  let doc = "Play the previous song in the playlist" in
  let man = [ `S Manpage.s_description; `P doc; `Blocks help_section ] in
...
let stop_t =
  let doc = "Stop playing song." in
  let man = [ `S Manpage.s_description; `P doc; `Blocks help_section ] in
  (Term.(const stop $ common_opts_t), Cmd.info "stop" ~doc ~sdocs ~exits ~man)
...
let pause_t =
  let doc = "Switch between play/pause." in
  let man = [ `S Manpage.s_description; `P doc; `Blocks help_section ] in
