...
let crossfade =
  ...
  let docv = "XFADE" in
  Arg.(
    value & opt (some int) None & info [ "xf"; "crossfade" ] ~docs ~doc ~docv)
...
let random =
  ...
  Arg.(
    value & opt (some bool) None & info [ "rand"; "random" ] ~docs ~doc ~docv)

let repeat =
  let doc =
    "Sets repeat state to REPEAT_STATE, REPEAT_STATE should be false or true."
  in
...
let single =
  let doc =
    "Sets single state to SINGLE_STATE, SINGLE_STATE should be 0 or 1. When \
     single is\n\
    \    activated, playback is stopped after current song, or song is \
     repeated if\n\
...
let mixrampdelay =
  ...
  Arg.(
    value & opt (some string) None & info [ "mixrampdelay" ] ~docs ~doc ~docv)
...
let mixrampdelay_wrapper client value =
  let string_parse str =
    ...
    | "nan" -> Pb_opt.Nan
    | _ -> (
        try Pb_opt.Seconds (float_of_string str) with Failure _ -> Pb_opt.Nan)
  in
  Pb_opt.mixrampdelay client (string_parse value)

let replay_gain_mode =
  let doc =
    "Sets the replay gain mode. One of off, track, album, auto with\n\
  ...
  Arg.(
    ...
    & info [ "replay_gain_mode" ] ~docs ~doc ~docv)
...
let replay_gain_mode_wrapper client value =
  let string_parse = function
    ...
    | _ -> Pb_opt.Auto
  in
  Pb_opt.replay_gain_mode client (string_parse value)
...
let cmd =
  let doc = "Configure all the playback options of the Mpd server." in
  let man = [ `S Manpage.s_description; `P doc; `Blocks help_section ] in
  ( Term.(
      const playback_options $ common_opts_t $ consume $ crossfade $ mixrampdb
      $ random $ repeat $ setvol $ single $ mixrampdelay $ replay_gain_mode),
