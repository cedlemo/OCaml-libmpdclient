...
let get_mpd_status_info status = function
  ...
  | Playlistlength ->
      [ "playlistlength:"; string_of_int @@ Mpd.Status.playlistlength status ]
  | State ->
  ...
  | Nextsongid ->
      [ "nextsongid:"; string_of_int @@ Mpd.Status.nextsongid status ]
  ...
  | Mixrampdb ->
      [ "mixrampdb:"; string_of_float @@ Mpd.Status.mixrampdb status ]
  | Mixrampdelay ->
      [ "mixrampdelay:"; string_of_float @@ Mpd.Status.mixrampdelay status ]
  | Audio -> [ "audio:"; Mpd.Status.audio status ]
  | Updating_db ->
      [ "updating_db:"; string_of_int @@ Mpd.Status.updating_db status ]
...
let get_status common_opts fields =
  ...
  | Ok status ->
      let rec _parse_fields = function
        | [] -> ()
        | i :: remain ->
            let info = String.concat " " @@ get_mpd_status_info status i in
...
let status_fields =
  ...
  let playlistlength =
    (Playlistlength, Arg.info [ "plistl"; "playlistlength" ])
  in
...
let cmd =
  let doc =
    "Get all status information with no arguments or chose those you want."
  in
  let man =
    [
