...
val delete:
  LwtClient.c -> int -> ?position_end:int -> unit -> Protocol.response Lwt.t
...
(** Moves the song at FROM or range of songs at START:END to TO in
    the playlist. *)
val move:
  LwtClient.c -> int -> ?position_end:int -> int -> unit -> Protocol.response Lwt.t
...
(** Moves the song with FROM (songid) to TO (playlist index) in the playlist.
    ...
    (if there is one). *)
val moveid:
  LwtClient.c -> int -> int -> Protocol.response Lwt.t
...
val playlistid:
  LwtClient.c -> int -> p Lwt.t

(** Finds songs in the current playlist with strict matching.*)
val playlistfind:
  LwtClient.c -> string -> string -> p Lwt.t
...
(** Shuffles the current playlist. START:END is optional and specifies a range
    of songs. *)
val shuffle:
  LwtClient.c -> ?range:(int * int) -> unit -> Protocol.response Lwt.t
...
(** Set the priority of the specified songs. A higher priority means that it
    ...
    songs is 0. *)
val prio:
  LwtClient.c -> int -> ?range:(int * int) -> unit -> Protocol.response Lwt.t

(** Same as prio, but address the songs with their id. *)
val prioid: LwtClient.c -> int -> int list -> Protocol.response Lwt.t

(** Swaps the positions of SONG1 and SONG2 (both song ids). *)
val swapid: LwtClient.c -> int -> int -> Protocol.response Lwt.t
...
val rangeid:
  LwtClient.c -> int -> ?range:(float * float) -> unit -> Protocol.response Lwt.t
...
*)
val cleartagid:
  LwtClient.c -> int -> string -> Protocol.response Lwt.t
