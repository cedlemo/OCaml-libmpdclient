...
val add:
  LwtClient.t -> string -> Protocol.response option Lwt.t

(** Adds a song to the playlist (non-recursive) and returns the song id.
    URI is always a single file or URL. For example: *)
val addid:
  LwtClient.t -> string -> int -> int Lwt.t
...
val delete:
  LwtClient.t -> int -> ?position_end:int -> unit -> Protocol.response option Lwt.t
...
(** Moves the song at FROM or range of songs at START:END to TO in
    the playlist. *)
val move:
  LwtClient.t -> int -> ?position_end:int -> int -> unit -> Protocol.response option Lwt.t
...
(** Moves the song with FROM (songid) to TO (playlist index) in the playlist.
    ...
    (if there is one). *)
val moveid:
  LwtClient.t -> int -> int -> Protocol.response option Lwt.t

(** Get a list of Song.s that represents all the songs in the current
    playlist. *)
val playlist:
  LwtClient.t -> t option Lwt.t

(** Get a list with the Song.s of the song id in the playlist *)
val playlistid:
  LwtClient.t -> int -> t option Lwt.t

(** Finds songs in the current playlist with strict matching.*)
val playlistfind:
  LwtClient.t -> string -> string -> t option Lwt.t
...
(** Shuffles the current playlist. START:END is optional and specifies a range
    of songs. *)
val shuffle:
  LwtClient.t -> ?range:(int * int) -> unit -> Protocol.response option Lwt.t
...
(** Set the priority of the specified songs. A higher priority means that it
    ...
    songs is 0. *)
val prio:
  LwtClient.t -> int -> ?range:(int * int) -> unit -> Protocol.response option Lwt.t

(** Same as prio, but address the songs with their id. *)
val prioid: LwtClient.t -> int -> int list -> Protocol.response option Lwt.t

(** Swaps the positions of SONG1 and SONG2 (both song ids). *)
val swapid: LwtClient.t -> int -> int -> Protocol.response option Lwt.t
...
val rangeid:
  LwtClient.t -> int -> ?range:(float * float) -> unit -> Protocol.response option Lwt.t
...
*)
val cleartagid:
  LwtClient.t -> int -> string -> Protocol.response option Lwt.t
