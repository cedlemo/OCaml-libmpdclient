...
val addid:
  Mpd.Client.c -> string -> int -> int

(** Clears the current playlist. *)
val clear:
  Mpd.Client.c -> Protocol.response
...
val delete:
  Mpd.Client.c -> int -> ?position_end:int -> unit -> Protocol.response

(** Deletes the song SONGID from the playlist. *)
val deleteid:
  Mpd.Client.c -> int -> Protocol.response

(** Moves the song at FROM or range of songs at START:END to TO in
    the playlist. *)
val move:
  Mpd.Client.c -> int -> ?position_end:int -> int -> unit -> Protocol.response
...
(** Moves the song with FROM (songid) to TO (playlist index) in the playlist.
    ...
    (if there is one). *)
val moveid:
  Mpd.Client.c -> int -> int -> Protocol.response
...
(** Shuffles the current playlist. START:END is optional and specifies a range
    of songs. *)
val shuffle:
  Mpd.Client.c -> ?range:(int * int) -> unit -> Protocol.response
...
(** Set the priority of the specified songs. A higher priority means that it
    ...
    songs is 0. *)
val prio:
  Mpd.Client.c -> int -> ?range:(int * int) -> unit -> Protocol.response

(** Same as prio, but address the songs with their id. *)
val prioid:
  Mpd.Client.c -> int -> int list -> Protocol.response
